...
public class ZookeeperClient {
                ...
                if (event.getState() == Event.KeeperState.SyncConnected) {
                    try {
                        ...
                        if (event.getType() == Event.EventType.None && event.getPath() == null) {
                            ...
                            ZookeeperClient.exitsZnode(event.getPath(), true);
    ...
    //检测节点是否存在
    public static void exitsZnode(String path, boolean watch) throws KeeperException, InterruptedException {
        if (zk != null) {
            //String path, boolean watch
            //检查节点路径.是否注册监听事件
            Stat exists = zk.exists(path, watch);
            if (exists == null) {
...
//            ZookeeperClient.deleteZnode("/zk-test1",5);//删除子节点之后，删除成功
            ...
            //6.检测节点是否存在
            ZookeeperClient.exitsZnode("/zk-test2", true);
